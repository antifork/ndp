...
c_link (int _argc, char **_argv, char *null)
{
  ...
  if ((rport_dyn = (u_short) atoi (_argv[2])) > 0)
	...
	if ((rport_dyn < ndp.conf.lp_irc) || (rport_dyn > ndp.conf.hp_irc))
	  {
	    send_msg (NULL,
		      "ndp: you are not allowed to connect that port\n");
	    return 0;
	  }
	break;
...
{
  char *cl[] = { "M", "U", "I", "i", "j", "^" }, *sip, *rip, null_ip[] =
    "-", me = '*', other = ' ';
  ...
  while (ptr != NULL)
      ...
      if (ptr->flag)
	{
	  ...
	  send_msg (NULL, "%s%c  %2d  %-15.15s %-15.15s%-5d %.25s...\n",
		    ...
		    ptr->fd_in, sip, rip, rpt, ptr->username);
...
item command[] = {
  ...
  {"w", c_who, CL_MASTER | CL_USER,
   "w,who                  - show users and filedescriptors"}
  ,
  {"iflist", c_ifbrowser, CL_MASTER | CL_USER | CL_IRC1,
   "iflist                 - show interfaces list"}
  ,
  {"kill", c_kill, CL_MASTER, "kill <fd>              - reset <fd> connection"}
  ,

  {"msg", c_msg, CL_MASTER | CL_USER,
   "msg  <fd> message      - send a message to <fd>"}
  ,
  {"link", c_link, CL_MASTER | CL_USER | CL_IRC1 | CL_IRC2,
   "link host:port:[vhost] - connect remote host"}
  ,

  {"exit", c_exit, CL_MASTER | CL_USER | CL_IRC1 | CL_IRC2,
   "exit,logout            - close link"}
  ,
  {"help", c_help, CL_MASTER | CL_USER | CL_IRC1 | CL_IRC2,
