...
create_dialer_sock (Channel * ptr, long vhost, long rhost, int rport)
{
      ...
      while ((k++ < 10) && ((ptr->fd_out = socket (AF_INET, SOCK_STREAM, 0)) == -1))
	    usleep (1000);
      ...
      if (ptr->fd_out == -1)
	{
      ...
      if (bind (ptr->fd_out, (struct sockaddr *) &(ptr->output_addr), sizeof (ptr->output_addr))==-1)
		send_msg(NULL,"bind: %s %s\n",multi_inet_nbotoa (ptr->output_addr.sin_addr.s_addr),strerror(errno));
...
halfshutdown_ (void)
{
      ...
      if (channel_ptr->fd_out)
	{
	      ...
	      while ((k++ < 10) && ((channel_ptr->fd_out = socket (AF_INET, SOCK_STREAM, 0)) == -1))
		    usleep (1000);
...
shutdown_ (Channel * parg)
{
      ...
      if (ptr->fd_out> 0) close (ptr->fd_out);
      if (ptr->fd_in > 0) close (ptr->fd_in);
