...
void fatalerr(char *,...) __attribute__((noreturn));
void
		...
		if ((p_chan->trial >= ndp.conf.maxerr) && (p_chan->state & S_LOGIN)) {
			...
			if (ndp.log_level) {
				syslog(LOG_INFO, "possible intruder from:%s with gecos irc %s!%s",
				       (char *) safe_inet_ntoa(p_chan->c_addr.sin_addr.s_addr),
...
handle_link(void)
{
	...
	if (FD_ISSET(p_chan->fd_in, &r_set) && FD_ISSET(p_chan->fd_out, &w_set)) {
		...
		if ((p_chan->opts & CH_UNKNOWN) && (ndp.opts & OPT_AUTO))
			stream_dissector(p_chan->bufout);
	...
	if (connect_out == 0) {
		...
		if (ndp.log_level & LOG_LEVEL2)
			syslog(	LOG_INFO, "%s[%s] linked to %s:%d",
			       	...
			       	safe_inet_ntoa(p_chan->c_addr.sin_addr.s_addr), 
				safe_inet_ntoa(p_chan->o_addr.sin_addr.s_addr), ntohs(p_chan->o_addr.sin_port));
...
server(void)
{
	...
	struct linger linger_ = {0, 0};
	int (*state_action[]) (void) = { accept_client, ushell, ushell, setup_dialer, connect_target, handle_link};
...
#define        free_chan        0x01

	{
		...
		chan_t *ptr_1 = NULL, *ptr_save = NULL;
...
main(int argc, char *argv[])
{
	...
	default:
		...
		fprintf(stderr, "\n--(%s on %s\n--(%s %s\n\n", VERSION, CPU_MFR_OPSYS, cvsid, MAIL_SUPPORT);
