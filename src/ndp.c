...
char            bufftmp[CHAN_SIZE_BUFF], *class[] = {"master", "user", "ircer+", "ircer", "ircer-", "unknown"};
...
accept_client(void)
{
	...
	if (Nchannel < (ndp.conf.n + 1)) {
		...
		channel_ptr->fd_in = accept(fd_in, (struct sockaddr *) & (channel_ptr->caller_addr), &callersize);

		if (channel_ptr->fd_in > 0) {

			setsockopt(channel_ptr->fd_in, SOL_SOCKET, SO_LINGER, (char *) &linger_, sizeof(linger_));
			setsockopt(channel_ptr->fd_in, SOL_SOCKET, SO_REUSEADDR, (char *) &opt, sizeof(opt));
			if (ndp.opts & OPT_CONF_) {
				{
					...
					snprintf(channel_ptr->prompt, CHAN_SIZE_PRPT - 1, "%s-(fd%d)# ",
						 pwd->pw_name,
						 channel_ptr->fd_in);
					rehash_time();
					return _LOGIN_;
...
ushell(void)
{
	...
	if ((channel_ptr->flag & _SHELL_) && (Nchannel > (ndp.conf.n + 1)) && !(channel_ptr->class & CL_MASTER)) {
	...
	if ((channel_ptr->trial >= ndp.conf.e) && (channel_ptr->flag & _LOGIN_)) {
		...
		if (ndp.log_level) {
			if (ndp.log_level)
			syslog(LOG_INFO, "possible intruder from:%s with gecos irc %s!%s", 
	(char *) multi_inet_nbotoa(channel_ptr->caller_addr.sin_addr.s_addr),
	...
	if (FD_ISSET(channel_ptr->fd_in, &r_set)) {
		...
		if (read_from_channel(channel_ptr->fd_in, channel_ptr->buffer_in) == -1)
			return _NULL_;
		...
		if ((last_char == 0x0a)) {
			...
			if (channel_ptr->flag & _LOGIN_) {
				...
				if (channel_ptr->class & ~CL_UNKNOWN) {
					...
					if ((ndp.opts & OPT_CONF_) && !(channel_ptr->class & CL_IRC3)) {
...
handle_link(void)
{
	...
	if (FD_ISSET(channel_ptr->fd_out, &r_set) && FD_ISSET(channel_ptr->fd_in, &w_set)) {

		rehash_time();

		if (!(channel_ptr->opts & (CH_IRC_ | CH_LINE_)) && !(ndp.opts & OPT_LINE_)) {
			...
			if (write_to_channel(channel_ptr->fd_in, channel_ptr->buffer_in, bt) == -1)
				return _READY_;
	...
	if (FD_ISSET(channel_ptr->fd_in, &r_set) && FD_ISSET(channel_ptr->fd_out, &w_set)) {

		rehash_time();

		if ((bt = read_from_channel(channel_ptr->fd_in, channel_ptr->buffer_out)) == -1)
			return _READY_;
		...
		if (write_to_channel(channel_ptr->fd_out, channel_ptr->buffer_out, bt) == -1)
			return _READY_;
...
setup_dialer(void)
{
	if (!(ndp.opts & OPT_CONF_)) {
		...
		if (ndp.vhost_4b)
			create_dialer_sock(channel_ptr, ndp.vhost_4b, ndp.rhost_4b, ndp.rport);
		else
...
connect_target(void)
{
	...
	if (diff_time(now.tv_sec, channel_ptr->sec) > _TIMEOUTCONN_) {
		...
		if ((channel_ptr->class == CL_IRC3) || (ndp.opts & (OPT_LOCAL_ | OPT_REMOTE_))) {
			shutdown_(NULL);
			return _READY_;
		} else {
			halfshutdown_();
	...
	if (connect_out == 0) {
		...
		setsockopt(channel_ptr->fd_out,
			   SOL_SOCKET,
			   SO_LINGER, (char *) &linger_,
			   sizeof(linger_));
		setsockopt(channel_ptr->fd_out, SOL_SOCKET, SO_REUSEADDR, (char *) &opt, sizeof(opt));
...
main(int argc, char *argv[])
{
	...
	if ((ndp.opts & OPT_CONF_) && (ndp.opts & (OPT_LOCAL_ | OPT_REMOTE_))) {
		fprintf(stderr, "ndp: -L-R and -c are mutually exclusive and cannot be used at the same time.\n");
		exit(-1);
	}
	if ((~ndp.opts | ~OPT_CONF_) >> 2 & (ndp.opts & OPT_REMOTE_) >> 1 & (~ndp.opts | ~OPT_LOCAL_) & 0x01) {
